console.log("[WebRTC] platform resolved -> .native.js");
...
const WebRTCView = forwardRef(function WebRTCView({ style, onWebRTCMessage }, ref) {
  ...
  const handleLoadEnd = useCallback(() => {
    setLoading(false);
    if (inited.current) {
      console.log("[WebRTC] init already requested, injecting init");
      webviewRef.current?.injectJavaScript(`window.init(); true;`);
    } else {
      console.log("[WebRTC] WebView loaded, waiting for permission");
    }
  }, []);
    ...
    <View style={[{ flex: 1 }, style]}>
      ...
      {error && (
        <View style={s.overlay}>
          ...
          <Text style={{ color: "#4ADE80", fontSize: 16, fontWeight: "bold" }} onPress={() => {
            ...
            webviewRef.current?.injectJavaScript(`window._hangUp(); window.init(); true;`);
      ...
      <WebView
        ...
        onPermissionRequest={(event) => {
          event.nativeEvent.grant(event.nativeEvent.permissions);
        }}
        onMessage={(event) => {
          try {
